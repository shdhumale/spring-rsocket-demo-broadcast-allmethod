...
//                .map(index -> new Message(SERVER, STREAM, index));
        ...
        while(ir.hasNext())
        {
        	...
        	rsocketRequester.route("send-from-server")
                    ...
                    .retrieveFlux(String.class)
                    .doOnNext(s -> logger.info("from send-from-server server side and message from the client in mono is ------------------>", s))
                    .subscribe();
//        	return Flux.interval(Duration.ofSeconds(60))
//                    .map(i -> new Notification(notification.getDestination(), notification.getSource(), "In response to inside iterator for send-from-server: " + notification.getText(), "Client Id:" + notification.getClientid()));
       }
